...
public class JavaMultithreadTest {
    ...
    /**
     * 我们知道创建线程的方式有两种，一种是实现Runnable接口，另一种是继承Thread，但是这两种方式都有个缺点，那就
     * 是在任务执行完成之后无法获取返回结果，那如果我们想要获取返回结果该如何实现呢？使用Callable和Future，通过
     * 它们构建的线程，在任务执行完成后就可以获取执行结果
     *
     * Callable<V>接口：
     * 定义：
     ...
     * }
     * 该接口声明了一个名称为call()的方法，同时这个方法可以有返回值V，也可以抛出异常。论是Runnable接口的实现类
     * 还是Callable接口的实现类，都可以被ThreadPoolExecutor或ScheduledThreadPoolExecutor执行，
     * ThreadPoolExecutor或ScheduledThreadPoolExecutor都实现了 ExcutorService接口， 而因此
     * Callable需要和Executor框架中的ExcutorService结合使用，ExecutorService提供的方法：
     ...
     * 因此我们只要创建好我们的线程对象（实现Callable接口或者Runnable接口），然后通过上面3个方法提交给线程池去
     * 执行即可。还有点要注意的是，除了我们自己实现Callable对象外，我们还可以使用工厂类Executors来把一个Runnable
     * 对象包装成Callable对象。
     * public static Callable<Object> callable(Runnable task)
     * public static <T> Callable<T> callable(Runnable task, T result)
     *
     * Future<V>接口：
     * Future<V>接口是用来获取异步计算结果的，说白了就是对具体的Runnable或者Callable对象任务执行的结果进行获取
     * (get()),取消(cancel()),判断是否完成等操作
     ...
     * V get(Long timeout , TimeUnit unit) ：获取异步执行结果，如果没有结果可用，此方法会阻塞，但是会有时间
     * 限制，如果阻塞时间超过设定的timeout时间，该方法将抛出异常。
     ...
     * 当任务已经启动，执行cancel(false)方法将不会对正在执行的任务线程产生影响(让线程正常执行到完成)，此时返回
     * false；当任务已经完成，执行cancel(...)方法将返回false。mayInterruptRunning参数表示是否中断执行中的线程。
     * 通过方法分析我们也知道实际上Future提供了3种功能：（1）能够中断执行中的任务（2）判断任务是否执行完成（3）
     * 获取任务执行完成后额结果。
     *
     * FutureTask类：
     ...
     * 如果任务取消成功，cancel(...)返回true；但如果执行cancel(false)方法将不会对正在执行的任务线程产生影响
     * (让线程正常执行到完成)，此时cancel(...)返回false。
    ...
    public void tryCallableTest(){
        ...
        Future<Integer> future = executorService.submit(new Callable<Integer>() {
            ...
            public Integer call() throws Exception {
                Log.d(TAG, "call: this is thread do sum");
                int sum = 0;
                Thread.sleep(2000);
                for (int i = 0; i < 500; i++) {
                    sum++;
                }
                Log.d(TAG, "call: this is thread do sum finish!");
                return sum;
            }
        });
        //执行完后关闭线程池
        ...
        try {
            if(future.get()!=null){
                Log.d(TAG, "tryCallableTest: we get the callable result is : " + future.get());
            }else {
                Log.d(TAG, "tryCallableTest: we can not get the callabel result!");
            }
    ...
    public void tryFutureTaskTest(){
        //此方法将runabe对象包装成callable
        Callable<Object> callable = Executors.callable(new MyTestRunable());
        ExecutorService executorService = Executors.newSingleThreadExecutor();
       ...
       executorService.shutdown();
        try {
            ...
            if(futureTask.get()!=null){
                Log.d(TAG, "tryFutureTaskTest: futureTask get the result is : " + futureTask.get());
            }else {
                Log.d(TAG, "tryFutureTaskTest: futureTask can not get the result");
            }
        }catch (Exception e){
